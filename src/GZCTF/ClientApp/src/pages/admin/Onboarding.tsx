...
const formatTime = (value?: string | number | null) => (value ? dayjs(value).format('DD MMM YYYY HH:mm:ss') : '-')
...
const GlobalOnboardingPage: FC = () => {
  ...
  return (
    <AdminPage>
      <Stack w="100%" className={adminClasses.configStack}>
        <div>
          <Title order={3}>Bulk Captain Onboarding</Title>
          <Text c="dimmed" size="sm">
            Provision tim berbayar, kirim link aktivasi ke captain, dan whitelist tim tersebut ke satu atau beberapa
            game sekaligus.
          </Text>
        </div>

        <Alert color="blue" title="Alur anggota tim tidak berubah" className={adminClasses.intro}>
          Captain membuat akun dari link email. Anggota lain tetap register secara normal, lalu memakai invite code tim
          bawaan GZCTF. Untuk warm-up dan final terpisah, pilih kedua game di bawah.
        </Alert>

        <SimpleGrid cols={{ base: 2, sm: 3, lg: 5 }} className={adminClasses.statusGrid}>
          {Object.values(CaptainOnboardingStatus).map((status) => (
            <Card key={status} withBorder padding="md" className={adminClasses.statusCard}>
              <Text size="xs" c="dimmed" tt="uppercase" fw={700}>
                {statusView[status].label}
              </Text>
              <Text size="xl" fw={800} c={statusView[status].color} className={adminClasses.statusValue}>
        ...
        <Stack className={adminClasses.section}>
          <Group justify="space-between" align="end" className={adminClasses.sectionHeading}>
            <div>
            ...
            <Group>
              ...
              <Tooltip label="Refresh sekarang">
                <ActionIcon variant="light" size="lg" loading={historyLoading} onClick={() => void loadHistory(true)}>
          ...
          <Table.ScrollContainer minWidth={1250}>
            <Table striped highlightOnHover withTableBorder className={adminClasses.ledger}>
              <Table.Thead>
                <Table.Tr>
                ...
                {history.map((record) => {
                  ...
                  return (
                    <Table.Tr key={record.inviteId}>
                      ...
                      <Table.Td>
                        <Group gap={4}>
                          {record.gameTitles.map((title) => (
                            <Badge key={title} size="sm" variant="light">
                              {title}
                            </Badge>
                          ))}
                        </Group>
                      ...
                      <Table.Td>
                        <Badge color={statusView[record.status].color}>{statusView[record.status].label}</Badge>
                      </Table.Td>
                      <Table.Td>
                      ...
                      <Table.Td>
                        ...
                        <Text size="xs" c="dimmed">
                          {formatTime(record.lastSentAtUtc)}
                        </Text>
                      </Table.Td>
                      <Table.Td>
        ...
        <Stack className={adminClasses.section}>
          <Title order={4} className={adminClasses.sectionHeading}>
            Kirim Batch Baru
          </Title>
